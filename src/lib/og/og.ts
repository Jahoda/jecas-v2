...
export function generateOgImage(post: OgPostData, options: OgOptions): Response {
	const { thumbnailUrl, tags = [], previewBranch } = options;
	...
	const displayDate = formatDate(post.last_modification || post.date || null);
	const readingTime = post.word_count ? calcReadingTime(post.word_count) : null;

	const isPreview = !!previewBranch;
	const backgroundGradient = isPreview
	...
	if (isPreview) {
		children.push({
			...
			props: {
				...
				children: `Preview: ${previewBranch}`
			}
		});
	}
