...
import { fetchTagContent, getTagFiles, clearCache } from '$lib/github/content';
import {
	getAllPosts as getMarkdownAllPosts,
	getSinglePostBySlug
...
const CACHE_DURATION = 60 * 1000; // 1 minute for ISR
...
async function loadAllTagFiles(): Promise<Map<string, Tag>> {
	...
	try {
		const tagFileNames = await getTagFiles();

		for (const fileName of tagFileNames) {
			const slug = fileName.replace(/\.md$/, '');

			try {
				const fileContent = await fetchTagContent(slug);
				const { data: frontmatter, content } = matter(fileContent);
				const htmlContent = await marked(content);
				const tag = normalizeTag(frontmatter as TagFrontmatter, htmlContent, slug);
				tags.set(tag.url_slug, tag);
			} catch {
				// Skip tag if fetch fails
			}
		}
