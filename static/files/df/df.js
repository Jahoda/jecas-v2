(function () { 
	window.onload = function () {
		var ram = document.getElementsByTagName("div")[2];
		...
		function nahled() {
			...
			nahled.id = "nahled";
			nahled.style.cssText = "position: fixed; background: #fff; overflow-y: auto; overflow-x: hidden; height: 100%";
			//nahled.innerHTML = "<h4>Náhled</h4>";

			var nahledKomentare = document.createElement("div");
			nahledKomentare.style.cssText = "position: fixed; right: 120px; top: 0; background: #fff; max-width: 50%; border: 2px solid #000; padding: 1em; display: none";
			...
			for (var i = 1; i < komentare.length - 1; i++) {
				...
				cara.style.cssText = "display: block; overflow: hidden; height: 3px; border: 1px solid #fff; width: 140px";
				cara.style.background = "black";
				cara.style.marginLeft =  20 + Math.round(odsazeni / 4) + "px";
				...
				cara.onmouseover = function () {
					...
					if (nahledKomentare.offsetHeight + this.offsetTop - nahled.scrollTop > (document.body.clientHeight)) {
			...
			nahledObal.style.cssText = "position: absolute; right: 0; top: 0; width: 120px; overflow: hidden; ";
		...
		function setActive() {
			...
			for (var i = 2; i < komentare.length - 1; i++) {
		...
		function zvyraznitNove() {
			var datum = new Date(inputDatum.value);
			var counter = 0;
			var nahledy = document.getElementById("nahled").getElementsByTagName("a");

			for (var i = 1; i < komentare.length - 1; i++) {
				var odkaz = komentare[i].getElementsByTagName("a");
				var datumPrispevku = parseDate(odkaz[1].innerHTML);
				var barva = "black";
				var vyska = "1px";
		...
		inputDatum.onchange = zvyraznitNove;		


		div.appendChild(inputDatum);
