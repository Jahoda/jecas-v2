var Obchod = (function () {
	...
	var odebratPolozku = function (polozka) {
		...
		prepnoutPolozku(
			document.getElementById(polozka.getAttribute('data-id')).parentNode,
			'data-vybrano'
		);
	};
	...
	var odeslat = function (formular) {
		...
		xhr.send(
			'zprava=' +
				encodeURIComponent(formular.zprava.value) +
				'&email=' +
				encodeURIComponent(formular.email.value) +
				'&cena=' +
				suma +
				'&objednavka=' +
				encodeURIComponent(elKosik.innerHTML)
		);
