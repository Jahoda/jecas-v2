...
var types = {
	...
	event: function () {
		return (
			...
			'\n' +
			'DTSTART:' +
			convertDate(f['event-start'].value) +
			'\n' +
			'DTEND:' +
			convertDate(f['event-end'].value) +
			'\n' +
			'END:VEVENT'
		);
	},
	...
	wifi: function () {
		return (
			'WIFI:T:' +
			radioValue(f['wifi-encrypt']) +
			';S:' +
			f['wifi-name'].value +
			';P:' +
			f['wifi-password'].value +
			';;'
		);
	},
	geo: function () {
	...
	vcard: function () {
		...
		return (
			...
			(f['vcard-email'].value.length
				? 'EMAIL;PREF;INTERNET:' + f['vcard-email'].value + '' + '\n'
				: '') +
...
function zobrazitQrKod(text) {
	smazatQrKod(qrObal);
	qrObal.appendChild(QRCode.generateHTML(text));
}
...
function createQrText(el) {
	var text = types[extractType(el.name)]();
	generateQrCode(text);
	rawData(text);
}
