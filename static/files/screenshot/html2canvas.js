...
(function (window, document, exports, global, define, undefined) {
		...
		function k(a, b, c, d) {
			...
			this.o(b)
				? ((this.m = b),
				  ...
				  0 === this.length
						? m(this.c, this.b)
						: ((this.length = this.length || 0), this.k(), 0 === this.d && m(this.c, this.b)))
				: g(this.c, this.p());
		}
		function h(a) {
			O++;
	...
	window.html2canvas = function (nodeList, options) {
		...
		var node = (
			nodeList === undefined ? [document.documentElement] : nodeList.length ? nodeList : [nodeList]
		)[0];
	...
	function crop(canvas, bounds) {
		...
		croppedCanvas
			.getContext('2d')
			.drawImage(canvas, x1, y1, x2 - x1, y2 - y1, bounds.x, bounds.y, x2 - x1, y2 - y1);
	...
	function cloneCanvasContents(ownerDocument, documentClone) {
		[].slice
			.call(ownerDocument.querySelectorAll('[' + html2canvasCanvasCloneAttribute + ']'), 0)
	...
	ImageLoader.prototype.isSVG = function (src) {
		return (
			src.substring(src.length - 3).toLowerCase() === 'svg' || SVGContainer.prototype.isInline(src)
		);
	};
	...
	function isPositionedForStacking(container) {
		...
		var zIndex =
			['absolute', 'relative', 'fixed'].indexOf(position) !== -1 ? container.css('zIndex') : 'auto';
		return zIndex !== 'auto';
	}
	...
	function nonIgnoredElement(nodeContainer) {
		return (
			...
			['SCRIPT', 'HEAD', 'TITLE', 'OBJECT', 'BR', 'OPTION'].indexOf(nodeContainer.node.nodeName) ===
				-1
		);
	}
