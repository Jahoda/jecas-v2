...
<p>Další příklad: AI agent (jako třeba <a href="/clawdbot">Clawdbot</a>), který automaticky zpracovává e-maily – třídí je, odpovídá na jednoduché dotazy a přeposílá důležité zprávy. Útočník pošle e-mail s textem:</p>
...
<p>Filtrujte a ošetřujte uživatelský vstup před předáním modelu. Nejúčinnější přístup je použít druhý, jednodušší model jako „strážce", který klasifikuje vstup ještě před tím, než ho předáte hlavnímu modelu. Strážce dokáže rozpoznat záměr útoku bez ohledu na jazyk, kódování nebo formulaci – na rozdíl od filtrování konkrétních frází (jako "ignoruj instrukce"), které útočník snadno obejde použitím jiného jazyka, synonym nebo Base64.</p>
...
<p>Prompt injection je nevyhnutelný důsledek toho, jak jazykové modely fungují – nedokážou spolehlivě rozlišit instrukce od dat. Na rozdíl od problémů jako SQL injection nebo XSS, které lze zcela eliminovat správným escapováním a validací, <b>proti prompt injection zatím neexistuje 100% algoritmické řešení</b>. Obranná opatření risiko výrazně snižují, ale ne na nulu.</p>

<p>Při návrhu aplikací s LLM je proto důležité zvážit, zda je zbytkové risiko přijatelné pro daný účel. Chatbot odpovídající na dotazy o produktech má jiný rizikový profil než agent s přístupem k e-mailům nebo platebnímu systému. Čím větší oprávnění model má, tím přísnější obranu potřebuje – a tím pečlivěji je třeba zvážit, jestli vůbec dává smysl mu taková oprávnění dát.</p>
